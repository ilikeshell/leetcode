...
class Solution91
{
    public int numDecodings(String s)
    {
        ...
        {
            //如果为1--9
            if(s.charAt(i - 1) >= 1 + '0' && s.charAt(i - 1) <= 9 + '0')
                dp[i] += dp[i - 1];

            //如果为10--26
            if(((s.charAt(i - 1) >= 0 + '0' && s.charAt(i - 1) <= 9 + '0') && s.charAt(i - 2) == 1 + '0')
                    ||((s.charAt(i - 1) >= 0 + '0' && s.charAt(i - 1) <= 6 + '0') && s.charAt(i - 2) == 2 + '0'))
                dp[i] += dp[i - 2];

            //如果dp[i] == 0，则两者都不是，直接返回
            if(dp[i] == 0)
                return 0;
        }
        return dp[s.length()];
